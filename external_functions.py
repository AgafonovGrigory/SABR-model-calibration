...
def Hagan_implied_vol(K: np.ndarray, F: np.ndarray, T: np.ndarray, alpha: float,
    ...
    """
    returns approximation for impied volatility in SABR model using Hagan formula
...
def SABR_Jacobian(K: np.ndarray, F: np.ndarray, T: np.ndarray, alpha: float,
    ...
    """
    returns derivatives of implied volatility with respect to SABR parameters
...
def generate_random_params() -> np.ndarray:
    """
    This function gerenates random parameters for SABR model
...
def SABR_proj(sabr_params: np.ndarray) -> np.ndarray:
    """
    This function projects sabr parameters to the valid range
    ...
    Parameters
        ----------
            ...
            f: callable 
                ...
                returns tuple res, J, where res is a numpy vector of residuals, 
                J is jacobian of residuals with respect to x 
            proj: callable
                function gets a vector of model parameters x, returns a vector of projected parameters 
            x0: np.ndarray
                initial parameters
